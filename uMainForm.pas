...
type
    ...
    procedure AdCountClick(Sender: TObject);
    ...
    procedure Chromium_OnBeforeResourceLoad(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest; const callback: ICefRequestCallback; out Result: TCefReturnValue);
...
begin
  ...
  begin
    ...
    ResourcesDirPath:='.\';
    FrameworkDirPath:='.\';
    AddCustomCommandLine('--ppapi-flash-path','pepflashplayer64_32_0_0_387.dll');
    AddCustomCommandLine('--ppapi-flash-version','32.0.0.387');
    AddCustomCommandLine('--plugin-policy','detect');
    IgnoreCertificateErrors:=true;
    OnContextInitialized := GlobalCEFApp_OnContextInitialized;
  end;
end;

procedure TMainForm.AdCountClick(Sender: TObject);
var TempChromium : TChromium;
begin
  If SearchChromium(PageControl1.TabIndex, TempChromium) then
  begin
    if EnableAdBlock = false then
    begin
      EnableAdBlock := true;
      AdCount.Caption := 'Ads: 0';
    end
    else
    begin
      EnableAdBlock := false;
      AdCount.Caption := 'Ads: (off)';
    end;

    TempChromium.Reload;
    TempChromium.SetFocus(True);
  end;
end;
...
begin
  ...
  TempChromium.OnBeforeResourceLoad := Chromium_OnBeforeResourceLoad;

  TempChromium.AdBlocked := 0;
...
procedure readAdList;
var adIndex: integer;
    f: text;
begin
	assignfile(f,'data/adlist.txt');
	{$I+}
	reset(f);
	{$I-}
	adIndex:=0;
...
var
	TempChromium : TChromium;
begin
  if (Key = 13) and SearchChromium(PageControl1.TabIndex, TempChromium) then
  begin
    if (pos('.', URLCbx.Text) = 0) or (pos(' ', URLCbx.Text) <> 0) then // không có dấu chấm hoặc có dấu cách thì search bình thường
      TempChromium.LoadURL('https://www.google.com/search?q=' + URLCbx.Text)
    else
...
begin
	if not(FClosing) and
     ...
     SearchChromium(PageControl1.TabIndex, TempChromium) and
...
procedure TMainForm.Chromium_OnBeforeResourceLoad(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const request: ICefRequest; const callback: ICefRequestCallback; out Result: TCefReturnValue);
var i: integer;
    TempChromium : TChromium;
    TempPageIndex : integer;
begin
  if EnableAdBlock AND GetPageIndex(Sender, TempPageIndex) then
  begin
    SearchChromium(TempPageIndex, TempChromium);
    
    for i:=0 to length(AdList)-1 do
    if (pos(AdList[i], request.Url)<>0) and (URLCbx.text<>'') and (pos(AdList[i], URLCbx.Text)=0) then
    begin
      inc(TempChromium.AdBlocked);
      if TempPageIndex = PageControl1.TabIndex then
        AdCount.Caption := 'Ads: ' + IntToStr(TempChromium.AdBlocked);
      Result:=RV_CANCEL;
    end;
  end;
